...
public class FileName {
	...
	/**
	 * Takes the path from the fully specified filename, goes &lt;levels&gt; directory levels up and searches the file beginning from there.<br>
	 * Result:<br>
	...
	public FileName(String filename) {
		...
		if (i > -1) { // found
			...
			if (filename.startsWith("http://")) {
				...
				filename = filename.substring(0, 2) + filename.substring(2).replace(':', '#'); // repace, if not drive descriptor
		}
	...
	public String getFileNameFromPath() {
		...
		if (i > -1)
			return absoluteFilename.substring(i + 1);
		else
	...
	public String getPathNameFromPath() {
		int i = absoluteFilename.lastIndexOf(sc);
		if (i > -1)
			return absoluteFilename.substring(0, i + 1);
		else
			return "";
	}
